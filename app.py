        ...
        if st.button("Cadastrar"):
            if new_u:
                ...
                try:
                    ...
                    conn.commit()
                    st.success(f"Usuário {new_u} cadastrado! Faça login.")
                except: st.error("Este nome já existe.")
                conn.close()
            else: st.warning("Digite um nome de usuário.")
else:
    # --- APP PRINCIPAL ---
    st.sidebar.title(f"⚽ {st.session_state.username}")
    menu = st.sidebar.radio("Navegação", ["Palpites", "Ranking", "Painel Admin"])
